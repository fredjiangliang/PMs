...
my ($warnMsgBody, $warnMsgHead, $die_MsgHead, $caller_inform)=@{ DieWork::BuildWarnDieHeadInform( 'Mutilple_PrintPNG.pl', 'Mutilple_PrintPNG.pl' ) };

my $welcomeMsg="
-h the input hash holding all the path of fasta files with muscle files
-p the input hash holding all the path of fasta files with interproscan files
...
my $inFstFl_interProscan_hash_file=$opt_p;                              DieWork::Check_FileDirExist_or_DIE( $opt_p,        "\$opt_p",        $die_MsgHead, $caller_inform  );  
...
my $inFstFl_interProscan_hash=Storable::retrieve ( $inFstFl_interProscan_hash_file ); DieWork::Check_Hash_or_DIE        ( $inFstFl_interProscan_hash, "\$inFstFl_interProscan_hash", $die_MsgHead, $caller_inform  );
 
my $out_HASH_FILE=$inFstFl_hash_file."_4m_.DomainsPng.hsh";                        if ( DieWork::Check_DfdNoEmptString_or_NOT( $opt_o )  ){ $out_HASH_FILE=$opt_o;  }       
...
foreach my $eachFastaFile (    @FastaFilePathArray    ){ warn "\n 20190415-0-0-3 \$eachFastaFile=$eachFastaFile\n";
  DieWork::Check_FileDirExist_or_DIE( $eachFastaFile,        "\$eachFastaFile",        $die_MsgHead, $caller_inform  );  
  
  DieWork::Check_FileDirExist_or_DIE ( $inFstFl_hash->{$eachFastaFile}->{'2m3m_delnum'}, "\$inFstFl_hash->{\$eachFastaFile}->{'2m3m_delnum'}=\$inFstFl_hash->{$eachFastaFile}->{'2m3m_delnum'}", $die_MsgHead, $caller_inform  );
  DieWork::Check_FileDirExist_or_DIE ( $inFstFl_interProscan_hash->{$eachFastaFile}->{'3m1m_itPrSc'}, "\$inFstFl_interProscan_hash->{\$eachFastaFile}->{'3m1m_itPrSc'}=\$inFstFl_interProscan_hash->{$eachFastaFile}->{'3m1m_itPrSc'}", $die_MsgHead, $caller_inform  );
  
  my $mutAlgnm_msf_file=$inFstFl_hash->{$eachFastaFile}->{'2m3m_delnum'};
  my $intPSCAN_hashFILE=$inFstFl_interProscan_hash->{$eachFastaFile}->{'3m1m_itPrSc'};
   
  my $mutAlgnm_DomainPNG         =$eachFastaFile."_4m1m_domain.png";  
 
  
  $outChunk_hash->{$eachFastaFile}->{'4m1m_domPng'}=$mutAlgnm_DomainPNG  ;
  
  #$each_muscle_msf, $each_interProScan_hash;
  ...
  if ($pid == 0) { ###### !!!!!!!#######
    ...
    my $intPSCAN_hash=Storable::retrieve ( $intPSCAN_hashFILE );              DieWork::Check_Hash_or_DIE ( $intPSCAN_hash, "\$intPSCAN_hash", $die_MsgHead, $caller_inform  );
  
    ClustalwRun::PrintPngForClustalw  ($mutAlgnm_msf_file, $intPSCAN_hash, $mutAlgnm_DomainPNG);  #, $msf_new_showNAMEhash);
