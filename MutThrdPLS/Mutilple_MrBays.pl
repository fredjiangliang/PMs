...
my ($warnMsgBody, $warnMsgHead, $die_MsgHead, $caller_inform)=@{ DieWork::BuildWarnDieHeadInform( 'Mutilple_MrBays.pl', 'Mutilple_MrBays.pl' ) };
...
my $out_HASH_FILE=$inFstFl_hash_file."_5m_.MrbayesOut.hsh";                        if ( DieWork::Check_DfdNoEmptString_or_NOT( $opt_o )  ){ $out_HASH_FILE=$opt_o;  }       
...
foreach my $eachFastaFile (    @FastaFilePathArray    ){ warn "\n 20190415-0-0-3 \$eachFastaFile=$eachFastaFile\n";
  ...
  my $mrBaysOUTdir     =$eachFastaFile."_5m_MB";  
 
  my $outNEXfilePATH=LGP_run_mrbayes::Not_Run_JUST_gettheNEXpath($mutAlgnm_msf_file, $mrBaysOUTdir);
  $outChunk_hash->{$eachFastaFile}->{'5m1m_MrBays'}=$outNEXfilePATH;
  ...
  if ($pid == 0) { ###### !!!!!!!#######
    ...
    my $outNEXfilePATH=LGP_run_mrbayes::Run_LGP_MRbayes($mutAlgnm_msf_file, $mrBaysOUTdir);
