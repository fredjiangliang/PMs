...
my ($warnMsgBody, $warnMsgHead, $die_MsgHead, $caller_inform)=@{ DieWork::BuildWarnDieHeadInform( 'Mutilple_buildIndex.pl', 'Mutilple_buildIndex.pl' ) };

my $welcomeMsg="
-h the input hash holding all the path of fasta files for indexing
...
my $out_HASH_FILE=$inFstFl_hash_file."_1m_.idx.hsh";                        if ( DieWork::Check_DfdNoEmptString_or_NOT( $opt_o )  ){ $out_HASH_FILE=$opt_o;  }       
...
foreach my $eachFastaFile (    @FastaFilePathArray    ){ warn "\n 20190415-0-0-3 \$eachFastaFile=$eachFastaFile\n";
  ...
  my $indexFile =$eachFastaFile."_1m1m_index.idx";  
  
  $outChunk_hash->{$eachFastaFile}->{'1m1m_indexF'}=$indexFile  ; 
  ...
  if ($pid == 0) { ###### !!!!!!!#######
    ...
    FastaFileHandle::BuildIdxFile_for_fastaFile($eachFastaFile, $indexFile);
