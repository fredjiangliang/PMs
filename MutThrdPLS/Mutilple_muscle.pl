...
my ($warnMsgBody, $warnMsgHead, $die_MsgHead, $caller_inform)=@{ DieWork::BuildWarnDieHeadInform( 'Mutilple_muscle.pl', 'Mutilple_muscle.pl' ) };

my $welcomeMsg="
-h the input hash holding all the path of fasta files for muscling
-c howmany core to use 
-m muscle fast running cutoff, if the sequence number > opt_m, than use the fast strategy
-o out put hash file
";
...
my $fastaCutLmt=400;                 if ( DieWork::Check_DfdNoEmptNUMBER_or_NOT( $opt_m )  ){ $fastaCutLmt=$opt_m;  } 

my $out_HASH_FILE=$inFstFl_hash_file."_2m_.mslOut.hsh";        if ( DieWork::Check_DfdNoEmptString_or_NOT( $opt_o )  ){ $out_HASH_FILE=$opt_o;  }       
...
foreach my $eachFastaFile (    @FastaFilePathArray    ){ warn "\n 20190415-0-0-3 \$eachFastaFile=$eachFastaFile\n";
  ...
  my $muscle_out_fasta_File_name =$eachFastaFile."_2m1m_MslOUT.fas.txt";    
  my $change_X_2_U_Muscle_MSF    =$eachFastaFile."_2m2m_chgX2U.fas.msf";    
  my $Del_number_befor_names_file=$eachFastaFile."_2m3m_delnum.fas.msf";    
  my $Changed_name_file          =$eachFastaFile."_2m4m_chgNam.fas.msf";     # not working here 
    
  
  $outChunk_hash->{$eachFastaFile}->{'2m1m_MslOUT'}=$muscle_out_fasta_File_name  ; 
  $outChunk_hash->{$eachFastaFile}->{'2m2m_chgX2U'}=$change_X_2_U_Muscle_MSF     ; 
  $outChunk_hash->{$eachFastaFile}->{'2m3m_delnum'}=$Del_number_befor_names_file ; 
  $outChunk_hash->{$eachFastaFile}->{'2m4m_chgNam'}=$Changed_name_file           ; 
  ...
  if ($pid == 0) { ###### !!!!!!!#######
    ...
    ClustalwRun::RunMuscle_and_obtain_varous_Format_alnFILE  ($eachFastaFile, $muscle_out_fasta_File_name, $change_X_2_U_Muscle_MSF, $Del_number_befor_names_file, $Changed_name_file, $fastaCutLmt);
